...
const getNews = async (user) => {
  if (user.rol === 'superadmin') return await newsRepository.findAllNews();
...
const getNewsById = async (id, user) => {
  ...
  if (!news) throw new Error('La noticia no existe');
...
const getPublicNewsDetail = async (countrySlug, newsSlug) => {
  if (!countrySlug || !newsSlug) throw new Error('El país y el slug de la noticia son obligatorios');

  const news = await newsRepository.findPublishedNewsDetailByCountryAndSlug(countrySlug, newsSlug);
  if (!news) throw new Error('Noticia no encontrada');
  return news;
};

const createNews = async (payload, user) => {
  const { titulo, resumen, contenido, imagen_principal_url, estado = 'borrador', pais_id } = payload;

  if (!titulo || !resumen || !contenido) throw new Error('Título, resumen y contenido son obligatorios');

  let finalPaisId = pais_id;
  if (user.rol !== 'superadmin') finalPaisId = user.pais_id;
  if (!finalPaisId) throw new Error('El país es obligatorio para crear una noticia');
...
const updateNews = async (id, payload, user) => {
  ...
  if (!existingNews) throw new Error('La noticia no existe');

  if (user.rol !== 'superadmin' && Number(existingNews.pais_id) !== Number(user.pais_id)) {
...
const uploadImage = async (id, file, user) => {
  const news = await newsRepository.findNewsById(id);
  if (!news) throw new Error('La noticia no existe');

  if (user.rol !== 'superadmin' && Number(news.pais_id) !== Number(user.pais_id)) {
    throw new Error('No tiene permisos para editar esta noticia');
  }

  const ext = file.originalname.split('.').pop().toLowerCase();
  const filePath = `noticias/${id}/imagen_${Date.now()}.${ext}`;
  ...
  const updated = await newsRepository.updateNews(id, {
    imagen_principal_url: publicUrl,
    updated_at: new Date().toISOString(),
  });

  return { message: 'Imagen subida correctamente', url: publicUrl, data: updated };
};
...
const deleteNews = async (id, user) => {
  ...
  if (user.rol === 'editor') throw new Error('El editor no tiene permisos para eliminar noticias');
