...
const createNews = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: 'Noticia creada correctamente', data: news });
  } catch (error) {
...
const updateNews = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'Noticia actualizada correctamente', data: news });
  } catch (error) {
    return res.status(400).json({ message: error.message });
  }
};
...
const toggleNewsStatus = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'Estado de la noticia actualizado correctamente', data: news });
  } catch (error) {
    return res.status(400).json({ message: error.message });
  }
};
...
const uploadNewsImage = async (req, res) => {
  try {
    ...
    const result = await newsService.uploadImage(id, req.file, req.user);
    ...
    auditService.register({
      ...
      modulo: 'noticias',
      registro_id: id,
      descripcion: `Imagen principal subida a noticia con id: ${id}`,
      ip: req.ip,
    }).catch(() => {});
